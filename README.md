...
- **Byproduct routes** — a recipe with a byproduct counts as a production route for the byproduct too: target Heavy Oil Residue and the planner builds Rubber refineries with the rubber as claimable surplus; target Plutonium Fuel Rods and the Uranium Waste arrives via actual nuclear plants. Routes appear alongside real recipes everywhere (pinnable "↩ via Rubber", listed in the resource panel and dependency trees) while the recipe itself lives — and is toggled — on its primary resource

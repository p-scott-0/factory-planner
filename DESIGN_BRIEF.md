...
- Edges that skip columns (long-range dependencies) routed via horizontal highway lanes above or below the node area so they don't overlap node boxes. **Forward** edges keep their position-based lane and run over the top; a **backward** edge (a producer that sits to the right of its consumer, e.g. a Water byproduct feeding an upstream Alumina Solution) always takes a bottom lane and loops **underneath** the machines rather than cutting back through them — so flow direction reads at a glance

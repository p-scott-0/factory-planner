...
### Byproduct routes
A recipe with a byproduct is a production route for the byproduct too — Rubber (3 Crude → 2 Rubber + 2 HOR) is equally a way to *make* Heavy Oil Residue, with rubber as the co-product. Rather than storing the recipe under both resources (two copies to keep in sync, split toggle identity), these views are **synthesized at planning time**: same machine and inputs, output = the byproduct amount, and the primary product joins the byproducts list — so the existing surplus/crediting machinery handles the co-product automatically (an HOR target grows a Rubber stage whose rubber shows as claimable surplus), and toggling the base recipe governs the route. Costing charges the FULL inputs to the byproduct unit — joint-cost allocation is inherently ambiguous, and undercharging would make byproduct routes spuriously beat direct recipes. Routes are pinnable ("↩ via Rubber"), appear in the resource list ("byproduct of Plastic, Rubber"), in the recipe panel as read-only entries (the toggle lives on the primary resource), and in dependency trees both as root choices in the tree picker and as amber "byproduct of X" nodes mid-tree. This is what finally made resources like Heavy Oil Residue (whose main sources are all co-products) and Uranium Waste (only a reactor byproduct) plannable without dashed pseudo-raw boxes.

### Recipe objectives
Automatic recipe selection has two objectives. **Cheapest** (Target mode's always, From-Inputs optional) minimises raw cost per unit, weighted by each raw's `rawCost`. **Optimal** (From-Inputs default) answers "what's the most target I can get from THESE inputs": supplied resources are priced at their real cost while everything else costs ~nothing, so the selector stretches the supplied inputs rather than the global raw budget — and because no per-unit pricing can force a chain to *consume* the supply at all (a chain that ignores it is free), when the auto-pick doesn't touch the supplied inputs the engine searches the target's enabled recipes directly, pinning each in turn and keeping whichever consuming chain achieves the highest rate. An explicit user pin always wins over that search (and errors honestly if its chain can't consume the supply). A resource that has recipes but none enabled/unlocked plans as an external raw — the raw box and raw-inputs row say so explicitly instead of impersonating a mineable input.

**Global route-mix optimization (From-Inputs).** Per-resource cost ranking is greedy — it picks one recipe per resource in isolation — and with a large recipe set, competing routes contesting the same feedstock can make the greedy pick (together with the surplus fixpoint) settle at a worse *equilibrium*: crude split between a Power primary and a fixed Plastic target lands higher when Plastic runs on the Power chain's Polymer Resin surplus (Residual Plastic) than when it buys crude, but no per-unit price can see that — it's a property of the whole mix. So after the greedy plan lands, the planner runs a bounded, deterministic hill-climb over the plan's *contested* resources (more than one enabled route, no user pin): forcing one resource's route at a time through the same pin machinery, re-running the full computation as the evaluation oracle, and keeping a change only when it strictly raises the primary target's achievable rate without breaking anything greedy had working (no new target errors or budget shortfalls). Accepted overrides stack across a few rounds under an evaluation cap. Greedy remains the fallback — ties change nothing — user pins are never contested, and a banner reports the gain and which routes were adjusted. The `cheap` objective skips the pass (it asks for the cheapest chain, not the most target).
...
### Named Plans, Pins, Overclocks, Budgets
Overclocks come in two levels: a **group clock** on each machine's results header applies to every stage of that machine in the plan, and an individual stage clock overrides it (setting a stage back to the group's value returns it to inheriting; explicit overrides show an accent border). In From-Inputs mode, **fixed-amount secondary targets carve their consumption out of the supplied budget first and the primary gets the remainder** — the amount field is the priority dial ("600/min Plastic; Power gets the rest of the crude"), with a warning when secondaries exhaust an input entirely. Blank secondaries still feed purely off the byproduct pool, which collects from fixed secondaries and the primary alike.

**Surplus flows into fixed targets too — in both directions.** A fixed target's recipe selection sees the plan's byproduct surplus as *free supply* (priced below everything, so Residual Plastic on leftover Polymer Resin beats any route that buys fresh inputs), and what it draws is netted out of the pool and the combined raw list, with a "♻ Fed from the plan's byproduct surplus" banner. In Target mode this flows forward (earlier targets' surplus feeds later fixed rows) in one pass. In From-Inputs mode the surplus mostly comes from the *primary*, which is computed **after** the carving — so the whole computation iterates to a fixpoint: each pass seeds the fixed secondaries with the previous pass's surplus, re-carves, re-plans the primary, and stops when the surplus map stabilises (real plans settle in 2–3 passes; a cap guards recipe flip-flops). The free-supply pricing is quantity-blind, so any draw the surplus can't fully cover is **produced by a supplemental chain** from the budget (e.g. surplus covers 1480/min of an 1800/min resin need → an Alternate: Polymer Resin stage makes the other 320) rather than surfacing as an unexplained external input.
The planner holds any number of **named saved plans** per profile; each plan owns its complete query — targets/inputs, **recipe pins** and **stage clocks** — so switching plans swaps the whole setup, and two builds can use different alternates without touching the profile-wide recipe toggles. A **pin** forces one recipe for a resource within that plan (falling back to auto if the pinned recipe is disabled or locked); toggles stay profile-level ("which alternates have I found"), pins are plan-level ("use this one here"). Any stage can be **overclocked** (1–250%): machines run proportionally faster, so fewer are needed, while each machine's power draw scales with clock^`powerExponent` — a profile setting, default 1 (linear), ≈1.321928 in the Satisfactory sample to match the game. Input/byproduct flows scale with the clock exactly like the output does.

**Compare** runs any two saved plans headlessly and renders a side-by-side table (per-target outputs, raw inputs, machine counts, total power). **Share links** encode the entire query into the URL hash (base64url, nothing server-side); opening one finds the matching profile (sample key first, then name), offers to load the sample if absent, and lands the query in a new "Shared plan" — the payload is treated as untrusted input and rebuilt field-by-field with types forced, so a crafted link can't inject markup. **Raw supply limits** live on the profile as usage data (editable inline in the results' raw-input rows, so they work on locked samples): when a plan's raw need exceeds a limit, the row highlights and a banner itemises the shortfall. Plans **export** as a standalone SVG of the flow diagram or a CSV of stages + raw inputs.
...
- **Column gaps are sized individually, not uniformly.** Each inter-column gap pays only for what lives in it: byproduct satellite boxes (196px when the left column has them), and a highway-descent zone when lane belts land in the right column — sized past the **widest rate pill in that column** plus staggered ramp room, so a descending lane belt always finishes left of every label and runs straight into the machine. Plain belt-only gaps stay at 128px. The diagram has −/+/Fit **zoom controls** (CSS-scaled, so hover tooltips keep working via the screen CTM), and oversized diagrams auto-fit once on touch devices.
- **Highway-affinity placement.** After crossing minimisation, nodes connected to the top highway (an output riding it, or an input arriving from it) are lifted toward the top of their column via whole-insertion moves, accepted lexicographically: the (all-pairs chord) crossing count may never get worse, and among equal-crossing layouts the summed slot-depth of lane-connected nodes is minimised — so ramps overlap fewer neighbours without ever trading away a crossing. Backward return belts get the same dynamic label-clearing lead-in as forward descents (finish the climb left of the column's widest pill, then straight in), and columns receiving backward arrivals reserve the same descent zone in their gap.
- **Node order within each column is chosen to minimise belt crossings.** Sorting by category then name reads tidily but ignores the graph, so belts criss-cross far more than they need to. Instead that sort is only the starting point: the diagram then runs the standard iterated **barycenter** heuristic over the layered graph — repeatedly reordering each column by the average position of each node's neighbours in the adjacent column, sweeping backward (order by producers) then forward (order by consumers), and keeping the best-scoring arrangement seen across the passes. Crossings are counted per column-pair from the node orderings, so the score is a property of the ordering rather than of the pixels. Byproduct supply links count as edges here just like primary outputs. Ties (a node with no neighbours in the sweep direction) hold their previous position, so the category/name sort still shows through wherever the graph doesn't care. In practice this roughly halves the crossings on real plans
- Edges that skip columns (long-range dependencies) routed via horizontal highway lanes above or below the node area so they don't overlap node boxes. Lanes are chosen by **direction**: **forward** edges always run over the **top**, a **backward** edge (a producer that sits to the right of its consumer, e.g. a Water byproduct feeding an upstream Alumina Solution) loops **under** — so flow direction reads at a glance. The backward entry is a single continuous hook (it stays going left and rises before curving forward into the machine's left edge, no abrupt reversal). Three refinements keep the ramps readable: **exit ramps stagger** by lane index (clamped to the clearance the source actually has — a byproduct box hangs in the inter-column gap with only ~50px before the next machines) so parallel long edges rise at different x instead of piling their curves onto one spot; **entry slots are ordered by arrival side** — top-highway edges take a machine's topmost input slots, adjacent-column edges the middle, under-highway edges the bottom — so a lane belt never crosses the machine face (or the rate pills of other entries) to reach a far slot; and **backward lanes are per-span** — an edge loops just under the deepest machine/byproduct box it actually crosses (overlapping spans push down a step), not under the entire diagram, so a one-column byproduct return no longer dives below the whole input stack. The reserved lane bands still **grow to fit** what a plan needs: the top band from the lane count, the bottom from the deepest per-span lane (both invariant across the re-render, so one retry always fits)
- Category-based colour coding on node borders
- Efficiency bands on nodes in from-inputs mode
- **Byproducts** branch off their producing node as a small, visually distinct satellite box (smaller than a machine box) labelled with the resource and rate, set out from the machine with a visible connector. A byproduct box hanging beside its machine attaches in the **bottom half** of the machine's right edge while the primary output belts are confined to the **top half**, so outputs and byproducts never overlap on the same edge (this generalises to any count of outputs/byproducts — each group is spread within its half). If another stage in the same plan consumes that byproduct, a normal edge runs from the satellite box to it; if nothing in the plan claims it, the box is dashed like a raw-input box to flag it as unused surplus. When a resource is produced by **both** a primary stage and as a byproduct (e.g. Silica from a Quartz constructor and from Alumina Solution), each consumer's draw is split proportionally between the two sources, so the edge rates add up to what the stages actually make. Byproduct supply counts toward column placement just like a primary-output dependency, so a producer whose byproduct feeds another node is positioned upstream (left) of that consumer — the connection reads left-to-right like every other flow rather than looping backward, even when the producer is itself a target
...
### Mobile
A real viewport meta plus phone-width media tiers: columns stack (results above the full-width flow diagram, which scrolls in its own card), nav and paddings tighten, and nothing about the desktop layout changes. The type-to-filter overlay is **document-anchored** (absolute, not fixed) so it stays glued to its select under pinch-zoom and scroll, clamps into the viewport when its select sits near the right edge, uses a 16px input font on touch devices (under that, mobile keyboards zoom the whole page), intercepts touchend/click as well as mousedown (mobile pickers open on those), swallows the synthetic click that trails a commit (it used to reopen the NATIVE picker over the just-chosen value), and skips refocusing the select on touch for the same reason.

### Planner Tab
The planner's chrome is collapsed by default so the content leads: the **Max Machine Tiers** and **Recipes** cards fold to one-line summaries (the Recipes card groups its rows under per-category headers, each independently collapsible, with the filter box always visible in the header — typing force-expands whatever matches and clearing hands back the user's own open/closed state). The results stage list is **grouped by machine type** — one line per machine with its total count, power and recipe count; expanding shows the per-recipe stages (groups holding a target start open). "Plan generation →" creates its own saved plan named for the wattage, leaving the plan it came from untouched; a power resource picked in any target row collapses the unit selector to a single **MW** option (power is continuous — "per minute" is meaningless for it).
Two sub-panels:
